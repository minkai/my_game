...
//same as FOR_LOOP_VECTOR?
#define FOR_LOOP( x ) for(int index = 0; index <= static_cast<int>( ( x ).size() )-1; index++)
#define FOR_LOOP_COUNT( x ) for(int count = 1; count <= static_cast<int>( ( x ).size() ); count++)
#define FOR_LOOP_VECTOR( x ) for(int index = 0; index <= static_cast<int>( ( x ).size() )-1; index++)
